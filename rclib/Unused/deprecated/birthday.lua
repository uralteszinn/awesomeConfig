...
bdayToString = function(bday)
  --if bday.howFar < 10 then
    --addSpace = " "
  --else
    --addSpace = ""
  --end
  --res = "[" .. addSpace .. bday.howFar .. "] " .. bday.name .. " <span color='grey80' font='7'>(" .. bday.nextBirthdayStr .. ")</span>"
  --return res
...
setBirthdayWidgets = function()
  birthdays = getBirthdays(wgt_path .. "birthdays")
  nextBirthdays = getNextBirthdays(birthdays)
  if table.maxn(nextBirthdays) ~= 0 and nextBirthdays[1].howFar == 0 then
    birthdayImage:set_image(gears.surface(beautiful.birthdayTodayIcon))
    birthdayTTIcon = beautiful.birthdayTodayTTIcon
  else
    birthdayImage:set_image(gears.surface(beautiful.birthdayIcon))
    birthdayTTIcon = beautiful.birthdayTTIcon
  end
  birthdayWidgetText:set_text(" " .. table.maxn(nextBirthdays) .. " ")
...
birthdayImage:connect_signal("mouse::enter", showBirthdayTT)
birthdayImage:connect_signal("mouse::leave", deleteBirthdayTT)
